!function () {
    ...
    $(`.send-captcha`).click(function () {
        message.prompt({
            title: '人机验证',
            width: 420,
            html: `<img src="/user/captcha/image?action=emailBindNew" data-acg-refresh="/user/captcha/image?action=emailBindNew" class="prompt-image-code" alt="${i18n('更换验证码')}">`,
            inputAttributes: {
    ...
    $('.save-data').click(function () {
        //改绑需登录密码二次验证（后端强制校验，F-33）：仅凭会话不足以改绑，防会话被窃后被改到攻击者邮箱。
        message.prompt({
            title: i18n('身份验证'),
            input: 'password',
            ...
            if (res.isConfirmed === true) {
                ...
                util.post("/user/api/security/email", data, () => {
